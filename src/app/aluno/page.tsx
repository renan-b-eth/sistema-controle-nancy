...
export default function AlunoDashboard() {
  ...
  useEffect(() => {
    ...
    // Iniciar registro de entrada
    const prepararEntrada = async () => {
      if (processado) return;
      
      let aula = getAulaAtual();
      
      // Se não houver aula (fora do horário), checa se o bypass está ativo
      if (!aula) {
        try {
  ...
  const registrarSolicitacao = async (u: any, aula: Aula) => {
    ...
    // Busca bypass config para validar se permite registro
    let isBypass = false;
    try {
      const res = await fetch('/api/adm/config');
      const config = await res.json();
      isBypass = config.bypass;
    } catch (e) {}

    if (horaAtual < 19 && u.ra !== '00000000000' && !isBypass) {
    ...
    try {
      const response = await fetch('/api/entradas', {
        ...
        body: JSON.stringify({ protocolo, aula_numero: aula.numero, horario: horarioAtual, data: dataAtual })
      });
  ...
  const atualizarAssinatura = async (status: 'assinado' | 'recusado') => {
    try {
      ...
      if (res.ok) setAssinaturaStatus(status);
    } catch (e) { console.error(e); }
  };
      ...
      <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
        <header className="flex justify-between items-center bg-card/70 backdrop-blur-xl p-6 rounded-[2rem] border border-border shadow-sm">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-xl font-black tracking-tight">PortãoEdu</h1>
              <p className="text-[9px] text-secondary font-black uppercase tracking-[0.2em]">Portal do Aluno</p>
            </div>
          </div>
          <button onClick={handleLogout} className="px-5 py-2.5 bg-red-50 text-red-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-100 transition-all border border-red-100">Sair</button>
        </header>
        ...
        <section className="relative overflow-hidden">
          {statusAtual === 'pendente' ? (
            ...
            <div className="p-8 sm:p-12 bg-emerald-500 text-white rounded-[3rem] shadow-2xl border-4 border-white animate-in zoom-in duration-500 relative overflow-hidden">
              ...
              <h2 className="text-3xl sm:text-4xl font-black uppercase mb-4 flex items-center tracking-tighter"><span className="mr-4 text-5xl">✅</span> LIBERADO!</h2>
              {assinaturaStatus === 'pendente' ? (
                <div className="bg-white/20 backdrop-blur-md p-6 sm:p-8 rounded-[2.5rem] border border-white/30 space-y-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm font-bold opacity-90 leading-relaxed">Dirija-se à coordenação para assinar o documento de entrada tardia <span className="underline font-black italic">manualmente (no papel)</span>. Após assinar, confirme sua entrada abaixo:</p>
                  </div>
                  <div className="bg-emerald-900/20 p-6 rounded-2xl border border-white/10 group cursor-pointer" onClick={() => setTermoAceito(!termoAceito)}>
                    <label className="flex items-start space-x-4 cursor-pointer">
                      <div className="relative flex items-center justify-center">
                        <input type="checkbox" checked={termoAceito} onChange={(e) => setTermoAceito(e.target.checked)} className="peer appearance-none w-6 h-6 border-2 border-white rounded-md checked:bg-white transition-all cursor-pointer" />
                        <span className="absolute text-emerald-600 font-black scale-0 peer-checked:scale-100 transition-transform">✓</span>
                      </div>
                      <span className="text-xs font-bold leading-tight select-none">Eu entendo que cheguei atrasado e afirmo que assinei o documento manualmente na coordenação. Compreendo o motivo do registro e aceito as políticas de frequência da escola.</span>
                    </label>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <button disabled={!termoAceito} onClick={() => atualizarAssinatura('assinado')} className={`py-5 rounded-2xl font-black uppercase text-xs transition-all shadow-xl ${termoAceito ? 'bg-white text-emerald-600 hover:scale-[1.02] active:scale-95' : 'bg-white/10 text-white/40 cursor-not-allowed'}`}>Confirmar Entrada</button>
                    <button onClick={() => atualizarAssinatura('recusado')} className="py-5 bg-emerald-700/30 text-white rounded-2xl font-black uppercase text-xs hover:bg-emerald-800 transition-all border border-white/10">Recusar / Problemas</button>
                  </div>
                </div>
              ) : (
                <div className="space-y-4 animate-fade-in">
                  <p className="text-2xl font-black mb-2 uppercase italic tracking-tighter">Acesso Confirmado!</p>
                  <p className="text-sm font-bold opacity-90 max-w-xl">{assinaturaStatus === 'assinado' ? 'Você confirmou a ciência dos termos e a assinatura manual. Prossiga para sua sala de aula.' : 'Você registrou uma recusa ou problema na assinatura. Procure a direção caso necessário.'}</p>
                  <div className="inline-block px-6 py-2 bg-white text-emerald-600 rounded-full text-[10px] font-black uppercase tracking-widest">Bom estudo!</div>
                </div>
              )}
            </div>
          ) : (
            <div className="p-8 sm:p-12 bg-red-600 text-white rounded-[3rem] shadow-2xl border-4 border-white animate-in slide-in-from-top duration-500 relative overflow-hidden">
              ...
              <h2 className="text-3xl sm:text-4xl font-black uppercase mb-4 flex items-center tracking-tighter"><span className="mr-4 text-5xl">⚠️</span> ATENÇÃO!</h2>
              <p className="text-xl font-black mb-4 uppercase">DIRIJA-SE À DIREÇÃO AGORA.</p>
              <p className="text-sm font-bold opacity-90">Sua entrada não pôde ser liberada automaticamente. Vá até a secretaria para regularizar seu acesso.</p>
            </div>
          )}
        </section>
